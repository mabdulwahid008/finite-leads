...
router.patch('/', authorization, masterOrAdminAuthorization, async(req, res) => {
    ...
    try {
        ...
        user = await db.query('UPDATE users SET name = $1, phone = $2, email = $3, address = $4, role = $5, brokerage_name = $6, broker_name  = $7, office_phone = $8, city = $9, country = $10, zip_code = $11, state = $12,  service_radius = $13, re_license_no = $14, rep = $15 WHERE _id = $16',[
            name, phone, email, address, role, brokerage_name, broker_name, office_phone, city, country, zip_code, state,  service_radius, re_license_no, rep, _id
        ])
...
router.get('/:role', authorization, masterOrAdminAuthorization, async(req, res) => {
    try {
        ...
        if(req.params.role != 99)
            users = await db.query('SELECT _id, name, email, phone, address, city, country, state, zip_code, service_radius, active, created_at, role FROM users WHERE role != 5 AND role = $1 AND active = 1 ORDER BY _id DESC',[
                req.params.role
            ])
        else
